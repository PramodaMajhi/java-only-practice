...
Second idea would be reverting the number itself, and then compare the
*  number with original number, if they are the same, then the number
* is a palindrome. However, if the reversed number is larger than
*  int.MAX, we will hit integer overflow problem.

Following the thoughts based on the second idea, to avoid the overflow
*  issue of the reverted number, what if we only revert half of the
int number? After all, the reverse of the last half of
*  the palindrome should be the same as the first half of the number,
*  if the number is a palindrome.

For example, if the input is 1221, if we can revert the last part of
* the number "1221" from "21" to "12", and compare it with the first
* half of the number "12", since 12 is the same as 12, we know that
...
All negative numbers are not palindrome, for example: -123 is not a palindrome
...
When the original number is less than the reversed number, it means
...
public class Palindrome {
	...
	public static void main(String[] args) {
		...
		System.out.println(isPalindrome(020));
	}

	public static boolean isPalindrome(int x) {
		// Special cases:
		// As we discussed above when x < 0, x is not a Palindrome.
		// Also if the last digit of the number is 0, in order to be Palindrome,
